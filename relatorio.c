...
void carrinho_para_relatorio_arquivo(carrinho compras[], int nTipoItens, float nProdutos, float tCompra)
{
    ...
    for( i = 0; i < nTipoItens; i++) {
        fprintf(f, "%d\n%s\n%f\n%f\n", compras[i].id, compras[i].nome, compras[i].quantidade, compras[i].preco);
        fprintf(f, "%d\n",data_atual());
    }
...
void imprime_relatorio (carrinho *compras, int n, int dataCompra)
{
    ...
    for(i = 0; i < n; i++) {
        for(j = 1; j < n; j++) {
            if (compras[j].id < compras[j-1].id) {
                aux = compras[j-1];
    ...
    for(i = 0; i < n; i++) {
        printf("\n%d   ", compras[i].id);
        printf("%s", compras[i].nome);
        espacamento_tabela(compras[i].nome, 33);
        printf("%.2f\t", compras[i].quantidade);
        printf("    R$ %.2f\t   R$ %.2lf\n",compras[i].preco, compras[i].preco * compras[i].quantidade);
        quantidade += compras[i].quantidade;
        total += (compras[i].preco * compras[i].quantidade);
    }
...
void ler_arquivo_relatorio()
{
    ...
    while(fscanf(f, "%d\n%[^\n]\n%f\n%f\n%d\n", &auxiliar.id, auxiliar.nome, &auxiliar.quantidade, &auxiliar.preco, &dataAuxiliar) == 5) {
        if(dataAuxiliar == dataProcurada) {
            if (n == 0) {
                ...
                compras = (carrinho*) malloc(sizeof(carrinho));
                if(compras == 0) printf("Erro no malloc");
                n++;
                compras[n-1] = auxiliar; //guarda o vetor na memoria alocada
            }	else {
                teste = 0;
                ...
                for(i = 0; i < n; i++) {
                    if (auxiliar.id == compras[i].id) {
                        teste = 1; // se teste for igual 1, então a id do produto já foi alocado em memória
                        compras[i].quantidade += auxiliar.quantidade;
                    }
                }
                //caso um novo produto seja encotrado, aloca memoria para o mesmo
                if(!teste) {
                    compras = (carrinho*) realloc(compras, (n+1) * sizeof(carrinho));
                    if(compras == 0) printf("Erro no realloc");
                    n++;
                    compras[n-1] = auxiliar; //guarda o vetor na memoria alocada
                }
            }
    ...
    imprime_relatorio(compras, n, dataProcurada);
}
