    ...
    while(1) {
        ...
        for(i = 0; ( i < (*qtd) ) && (!teste2); i++) {
            if (aux == prods[i].id) {
                ...
                total_compra += prods[i].preco * qtd_produtos; //aumenta o total da compra
                ...
                compra[c-1].id = prods[i].id;
                strcpy(compra[c-1].nome,prods[i].nome);
                compra[c-1].preco = prods[i].preco;
